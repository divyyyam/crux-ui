...
export default function AiChatbotScreen() {
  ...
  return (
    <SafeAreaView className={`flex-1 ${isDark ? 'bg-[#000000]' : 'bg-[#FAFAFA]'}`}>
      ...
      >
        {/* Header */}
        <View className={`px-6 py-4 border-b ${isDark ? 'border-[#222222]' : 'border-gray-100'} flex-row items-center border-t-0`}>
          <View className={`w-10 h-10 rounded-full items-center justify-center mr-3 ${isDark ? 'bg-[#111111]' : 'bg-gray-100'}`}>
        ...
        <ScrollView className="flex-1 px-4 pt-6">
          ...
          <View className="flex-row justify-end mb-6">
            <View className={`max-w-[80%] rounded-3xl rounded-tr-sm px-5 py-4 ${isDark ? 'bg-[#222222]' : 'bg-gray-200'}`}>
          ...
          <View className="flex-row justify-start mb-6">
            <View className={`w-8 h-8 rounded-full items-center justify-center mr-2 mt-1 ${isDark ? 'bg-[#111111]' : 'bg-gray-100'}`}>
              <Sparkles size={14} color={isDark ? '#39FF14' : '#000000'} />
            </View>
            <View className={`max-w-[80%] rounded-3xl rounded-tl-sm px-5 py-4 ${isDark ? 'bg-[#111111] border border-[#222222]' : 'bg-white border border-gray-100 shadow-sm'}`}>
        ...
        {/* Input Area */}
        <View className={`px-4 pt-4 pb-4 border-t ${isDark ? 'bg-[#000000] border-[#222222]' : 'bg-[#FAFAFA] border-gray-100'}`}>
          <View className={`flex-row items-center px-2 py-1 rounded-full border ${isDark ? 'bg-[#111111] border-[#333333]' : 'bg-white border-gray-200'}`}>
            <TextInput 
              ...
              multiline
            />
            <TouchableOpacity className={`w-10 h-10 rounded-full items-center justify-center mr-1 ${isDark ? 'bg-[#39FF14]' : 'bg-black'}`}>
